...
def mutate_character(
    ...
    random_new_ability_probability: float
):
    char_mutated_props = {
    ...
    melee_ability_data = {
        prop_name: {'range': data.value_range, 'type': data.property_type} for prop_name, data in melee_property_data.items()
    }

    projectile_ability_data = {
        prop_name: {'range': data.value_range, 'type': data.property_type} for prop_name, data in projectile_property_data.items()
    }

    for i in range(len(abilities_mutated_props)):
        if random.random() < random_new_ability_probability:
            abilities_mutated_props[i] = generate_random_ability(
                melee_ability_data, projectile_ability_data)
...
def generate_random_ability(melee_ability_data, projectile_ability_data):
    new_ability = {prop_name:
                   values['type'](random.uniform(
                       values['range'][0], values['range'][1]))
                   for prop_name, values in (
                       melee_ability_data if random.random() > 0.5 else projectile_ability_data
                   ).items()
                   }
    print(new_ability)
    return new_ability
...
@dataclass
class RandomNewAbilityMutation(Mutation):
    random_new_ability_probability: float
    character_property_data: Dict[str, PropertyMutationData]
    melee_property_data: Dict[str, PropertyMutationData]
    projectile_property_data: Dict[str, PropertyMutationData]
    ...
    def __call__(self, original_genome: Individual) -> Individual:
        ...
        mutated_char_config: CharacterConfig = mutate_character(
            ...
            random_new_ability_probability=self.random_new_ability_probability
        )
